...
\subsection{Clustering Base Openings by Probability of Game Results}
Following our earlier exploration of the most popular base openings, we investigated whether we could cluster them by the probability of game results. Accurate clusters would allow us to identify base openings that are more likely to result in a certain outcome -- this could be useful in determining someone's style of play, thus opening the possibility for more fine-tuned analysis of match-ups between players.

\subsubsection{Calculating Probability of Game Results Per Base Opening}
Before applying our clustering algorithm, we needed to aggregate the data to obtain the probability of game results per base openings. This involved counting the number of outcomes for each base opening, and then calculating the proportions of each outcome. Note that we filtered the data to only include base openings with a significant sample size to remove outliers and thus improve the quality of our clustering. For our analysis of games across all ratings, we required a minimum of 10,000 games, whereas for high, middle, and low rated players we required a minimum of 1,000 games. Figure \ref{fig:probabilityOfGameResultsForTop15BaseOpenings} shows the probability of game results for the top 15 base openings, sorted in descending order of the number of games.

\begin{figure}[H]
    \centering
    \caption{Probability of Game Results for Top 15 Base Openings}
    \label{fig:probabilityOfGameResultsForTop15BaseOpenings}
    \includegraphics[width=0.8\textwidth]{Probability of Game Results for Top 15 Base Openings.png}
\end{figure}

\subsubsection{Implementing K-Means Clustering}
\begin{figure}[H]
    \centering
    \caption{Base Openings Clustered by Probability of Game Results}
    \label{fig:baseOpeningsClusteredByProbabilityOfGameResults}
    \includegraphics[width=0.6\textwidth]{Base Openings Clustered by Probability of Game Results.png}
\end{figure}
