...
\subsubsection{Results of the Decision Tree and Random Forest Classifiers}
...
\subsubsection{Results of the Linear Regression Model for All Players}
...
\subsubsection{Results of the Linear Regression Model for Different Rating Groups}

When we generated the linear regression models for each rating group, we found that there were similarly bad metrics for the high and mid rating groups. Figure \ref{fig:metricsForLinearRegressionModelsOfPopularityOfBaseOpeningVsGameResults} shows a comparison table showing the R2 score and mean absolute error for each one. However, we noticed that at the low rating group, the R2 scores and mean absolute errors were significantly better.
...
\begin{figure}[H]
    ...
    \begin{tabular}{| l | l | l | l |} 
        \hline
        \bf{Rating Group} & \bf{Game Result} & \bf{R2 Score} & \bf{MAE} \\ [0.5ex] 
        \hline
