...
To evaluate our clustering, we will use the silhouette score, which is a measure of how similar an object is to its own cluster compared to other clusters. It is defined as $\frac{b - a}{max(a, b)}$, where $a$ is the mean distance between an object and all other objects in the same cluster, and $b$ is the mean distance between an object and all other objects in the next nearest cluster. The silhouette score ranges from -1 to 1, where a score close to 1 indicates that the object is well matched to its own cluster and poorly matched to neighbouring clusters, and a score close to -1 indicates the opposite.
