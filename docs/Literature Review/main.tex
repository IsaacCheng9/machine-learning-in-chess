...
% \textnormal{
\noindent 
Chess is a popular strategy game that has seen tremendous growth over the past decade. As the game has grown, so has the amount of data available to study. In this paper, we conduct a critical review into the historical approaches to computer chess, and the current state of the art in machine learning for chess. We then present a project plan to explore the use of machine learning to understand the patterns in how people play chess, influenced by the challenges discussed in the review.
...
\begin{itemize}
    ...
    \item \textbf{How good are existing metrics for predicting win probability in chess games?} I believe that centipawn score is a good predictor of win probability, but that other metrics such as the number of pieces on the board and the number of moves played are also good predictors.
